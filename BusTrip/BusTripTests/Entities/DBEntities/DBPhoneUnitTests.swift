...
final class DBPhoneUnitTests: XCTestCase {
    ...
    func testInit() {
        ...
        // When
        let response = ReportMock().generateDBReport(name: name,
                                                     surname: surname,
                                                     email: email,
                                                     phone: phone,
    ...
    func testConvertToEntity() {
        ...
        // Given
        let request = ReportMock().generateDBReport()
        ...
        XCTAssertEqual(response.surname, request.surname)
        XCTAssertEqual(response.email, request.email)
        XCTAssertEqual(response.phone, request.phone)
        XCTAssertEqual(response.reportTime, request.reportTime)
        XCTAssertEqual(response.reportDescription, request.reportDescription)
    }
}
